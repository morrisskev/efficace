...
def distances_P():
    ...
    (distanceArbPlayers, distancePlayersArb) = get_distance_arb__players(
        adj_list, (num_players+1))
...
def triDistance_P():
    ...
    distances_P()
    j = 0
    # On copiera dans cette liste les sommes des arbitres avec chaque joueur en ajoutant pour chaque somme le numéro du joueur
    listWithNumPlayers = []
...
def getResult():
    ...
    # On place dans team1 le joueur avec la communication la plus forte et dans team2 le deuxième joueur avec la communication la plus forte
    (team1, team2) = getFirstTeams(sumOfArbPlayrs)
    (chefEq1, tmp1) = team1[0]
    (chefEq2, tmp2) = team2[0]
    e1 = 0
    e2 = 0
    listTriee = triDistance_P()
    (distance_arb_players, distance_players_to_arb) = get_distance_arb__players(
        adj_list, (num_players+1))
...
def parse(index):
    ...
    to_open = ""
    if index < 9:
        to_open = "0" + str(index) + ".in"
    else:
        to_open = str(index) + ".in"
    print("Opening file : " + to_open)
...
if __name__ == "__main__":
    ...
    # print(triDistance_P())
    for i in range(1, 51):
        print("Now working on file number : ", i)
        parse(i)
        res = getResult()
